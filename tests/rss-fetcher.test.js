...
describe('RssFetcher Integration', () => {
  ...
  describe('fetchFeed', () => {
    test('should fetch and parse RSS feed successfully', async () => {
      ...
      // Mock the got-scraping call
      jest.doMock('got-scraping', () => ({
        gotScraping: jest.fn().mockResolvedValue(mockResponses.rssSuccess),
      }));
    ...
    test('should handle empty RSS feed', async () => {
      jest.doMock('got-scraping', () => ({
        gotScraping: jest.fn().mockResolvedValue(mockResponses.emptyRss),
      }));
    ...
    test('should handle malformed RSS feed', async () => {
      jest.doMock('got-scraping', () => ({
        gotScraping: jest.fn().mockResolvedValue(mockResponses.malformedRss),
      }));
    ...
    test('should handle network errors', async () => {
      jest.doMock('got-scraping', () => ({
        gotScraping: jest.fn().mockRejectedValue(new Error('Network error')),
      }));
  ...
  describe('processRssItems', () => {
    ...
    test('should handle items without guid', () => {
      const items = [
        { title: 'No GUID Item', link: 'https://example.com/no-guid' },
        global.testUtils.createMockRssItem({ guid: 'item1' }),
      ];
      
      const newItemsCount = rssFetcher.processRssItems(items);
      
      expect(newItemsCount).toBe(1); // Should skip item without guid/link
